...
function About() {
  ...
  return (
    <>
      <section id="secHead">
        <div id="head1">
        <p ref={p1ref} style={{fontWeight:"bolder"}}>RESUME</p>
        </div>
        <div id="head2">
        <p ref={p2ref} style={{fontWeight:"bolder"}}>ABOUT <span>ME</span></p>
        </div>
      </section>
...
{data.map(({FirstName,Age,Freelance,Phone,Skype},i)=>{
  ...
  <p>Age : {Age}</p>
  <p>Freelance : <span style={{color:"rgb(8, 189, 8)"}}>{Freelance}</span></p>
  <p>Phone : {Phone}</p>
  <p>Skype : {Skype}</p>
  </React.Fragment>
})}
...
{/* LAST NAME COLUMN */}
<div id='lastName'>
{data.map(({LastName,Nationality,Address,Email,Languages},i)=>{
  return <React.Fragment key={i}>
...
<a id='download' ref={p4ref} href={resume} target='_blank'>
<p>DOWNLOAD CV</p>
<div id='Dsvg'>
<FaDownload/>
</div>
